  ...
  const handleDetect = async () => {
    ...
    for (const user of users) {
      ...
      try {
        ...
        // Re-auth if credentials older than 5 days
    ...
    for (const { user, activity } of readyUsers) {
      ...
      try {
        ...
        recordSign({
          ...
          type: 'normal', otherId: Number(activity.otherId) || 0,
        ...
        <CardContent>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, mb: 2 }}>
            <Typography variant="body1">
              {users.length} {t('account.title').toLowerCase().replace('account management', 'users')}
            </Typography>
          </Box>
